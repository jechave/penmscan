...
test_that("historically colliding pairs get distinct seeds and distinct draws", {
  ...
  seeds <- vapply(pairs, function(p) penmscan:::mut_seed(241956, p[1], p[2]), numeric(1))
...
test_that("a full scan produces no colliding seeds", {
  ...
  keys <- mapply(function(j, m) penmscan:::mut_seed(241956, j, m), g$j, g$m)
...
test_that("a mutant's stream does not depend on nmut", {
  ...
  draw <- function() { set.seed(penmscan:::mut_seed(1024, 80, 3)); rnorm(3) }
  expect_equal(draw(), draw())

  keys10 <- vapply(seq(10), function(m) penmscan:::mut_seed(1024, 80, m), numeric(1))
  keys50 <- vapply(seq(50), function(m) penmscan:::mut_seed(1024, 80, m), numeric(1))
...
test_that("different seeds give disjoint scans", {
  ...
  g <- expand.grid(j = seq(228), m = seq(10))
  k1 <- mapply(function(j, m) penmscan:::mut_seed(1, j, m), g$j, g$m)
  k2 <- mapply(function(j, m) penmscan:::mut_seed(2, j, m), g$j, g$m)
...
test_that("the two sdmrs ensembles are disjoint", {
  ...
  g <- expand.grid(j = seq(228), m = seq(10))
  e1 <- mapply(function(j, m) penmscan:::mut_seed(1024, j, m, 1L), g$j, g$m)
  e2 <- mapply(function(j, m) penmscan:::mut_seed(1024, j, m, 2L), g$j, g$m)

  expect_equal(length(intersect(e1, e2)), 0L)
})

test_that("check_seeds_distinct() fails loud on a collision", {
  expect_silent(penmscan:::check_seeds_distinct(c(1L, 2L, 3L)))
  expect_error(penmscan:::check_seeds_distinct(c(1L, 2L, 1L)), "Seed collision")
})
