...
### 6b. `--as-cran` network checks make `check()` look like it hangs

**A separate problem from 6a** — `repos` was set correctly and this still
happened. `devtools::check()` defaults to `--as-cran`, which performs network
lookups. On a machine that cannot reach those endpoints each one blocks until it
times out: measured **413s wall-clock for ~47s of actual work**, i.e. ~6 minutes
of pure waiting. The world-clock check alone is a 60s timeout.

Fixes:

- `_R_CHECK_SYSTEM_CLOCK_=0` in `~/.Renviron` kills the 60s clock timeout.
  (Set on this machine 2026-08-13; the file did not previously exist.)
- Use `devtools::check(cran = FALSE)` for routine milestone checks — **413s → 53s**
  here. Save `--as-cran` for actual CRAN submission, ideally on CI where the
  network checks can really run.

**Caveat when comparing runs:** the counts are not comparable. `cran = FALSE`
suppresses some checks, so the fast run reports 2 warnings / 2 notes where the
full `--as-cran` run reports 3 / 3. Neither figure is wrong; they measure
different things. All findings under both are the pre-existing documentation
issues in §3 and §4 above.
